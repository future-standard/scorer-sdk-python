...
#ZMQ scoket String
img_sock      = "ipc://@/scorer/frame_grabber-video0"
log_sock      = "ipc://@/scorer/logger-marker"
sdk_sock      = "ipc://@/scorer/logger-sdk"
web_sock1      = "ipc://@/scorer/web-sdk1"
web_sock2      = "ipc://@/scorer/web-sdk2"
web_sock3      = "ipc://@/scorer/web-sdk3"
web_sock4      = "ipc://@/scorer/web-sdk4"
MAX_WEB_SCOCK  = 4
...
class Frame:
    """Frame class. This class handles frame and date data.
    """
    ...
    def get_col(self):
        ...
        :return: column number of this image
        """
        return self.my_col[0]

    def get_row(self):
        """Get row number

        :return: row number of this image
        """
        return self.my_row[0]
...
class Scorer:
    ...
    def __init__(self, task, roi_file='default'):
        ...
        #
        self.log_sock = ctx.socket(zmq.PUB)
        self.log_sock.setsockopt(zmq.SNDHWM, 10000)
        self.log_sock.connect(log_sock)
        #
        ...
        try:
            while True:
                ...
                if self.img_sock in socks and socks[self.img_sock] == zmq.POLLIN:
                    ...
                    self.frame = Frame(timestamp, format, rows, cols, mat_type, data)
                    self.count = 1
        except:
            if self.count == 0:
                return (None)
