...
COLLECTION_ID = 'test'
AWS_ACCESS_KEY_ID = 'AKIAIM4VE6A7HTXURZFA'
AWS_SECRET_ACCESS_KEY = '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'
MAX_RESULTS = 20
CUT_WIDTH_MARGIN = 20
...
class face_indexer:
	def __init__(self, save_logs_, save_snapshots_, imshow_):
		...
		self._data_collect = data_collector(True)
			...
			if objectid == None:
				...
				if os.path.isdir(NAME_PATH):
					...
					if len(images_path) != 0:
						self._data_collect.enter_data_input(self._objectid - 1, datetime, int(age_ave), int(age_width), gender)
			else:
				#print(objectid)
				#print(datetime)
				#print(int(age_ave))
				...
				if os.path.isdir(NAME_PATH):
					...
					if len(images_path) != 0:
						self._data_collect.enter_data_input(objectid, datetime, int(age_ave), int(age_width), gender)
					...
					try:
						...
						if len(faceMatches) > 0:
							...
							if os.path.isdir(NAME_PATH):
								...
								if len(images_path) != 0:
									datetime = '%04d-%02d-%02dT%02d:%02d:%02d' % (t.year, t.month, t.day, t.hour, t.minute, t.second)
									self._data_collect.exit_data_input(objectid, datetime)
