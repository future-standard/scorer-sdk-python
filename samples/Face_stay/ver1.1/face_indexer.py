				...
				if (cut_size[2] - cut_size[3]) * (cut_size[0] - cut_size[1]) > FACE_MINIMUM_SIZE:
					...
					try:
						...
						if len(faceMatches) > 0:
							...
							if self._save_logs:
								self._regi_logger.log(20, 'detect:' + str(objectid) + ' enter:' + str(time) + ' exit:' + str(obj._last_seen))
							...
							if self._save_snapshots:
								...
								cv2.imwrite(imgname, dst)
								t = time
								entertime = '%04d-%02d-%02d %02d:%02d:%02d' % (t.year, t.month, t.day, t.hour, t.minute, t.second)
								t = obj._last_seen
