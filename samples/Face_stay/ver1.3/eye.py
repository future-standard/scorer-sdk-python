...
RTSP = "rtsp://admin:cameleon2015@192.168.1.13"


#def lost(face_index, data_collect, lost_list):
#    face_index.face_search_enter(lost_list)
#    for value in face_index.result():
#        data_collect.enter_data_input(value[0], value[1])
        

if __name__ == '__main__':
    threads = []
    video_capture = video_capturer("scorer")
    eye_detect = eye_detector(DETECT_SAVE_LOGS)
    ...
    try:
        while True:
            eye_detect.detect(*video_capture.get_image())
            object_track.track_objects(*eye_detect.result())
