...
class face_detector:
	...
	def detect(self):
		...
		while frame == None:
			...
			self.time = frame.datetime

		# Get bgr and gray
		self.bgr = frame.get_bgr()
		gray = frame.get_gray()

		# Create thumbnail
		scale = 1.0 * MOG2_DETECT_IMAGE_WIDTH / frame.width
		...
		if self._save_logs and len(self._detected) > 0:
			self._logger.log(20, frame.datetime)
